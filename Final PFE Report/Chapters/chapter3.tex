...
RabbitMQ is a message-queueing software also known as a message broker or queue manager. Simply put, it is software where queues are defined, to which applications connect in order to transfer messages \cite{rabbitmq}. We utilized RabbitMQ to handle asynchronous tasks and background processing. It supports both message queuing and publish/subscribe messaging patterns, ensuring reliable message delivery, fault tolerance, and scalability. This makes RabbitMQ an excellent choice for decoupling system components and managing distributed systems. In our implementation, RabbitMQ orchestrates tasks, events, and notifications, ensuring efficient and dependable communication between various services.
...
Combining Vite with TypeScript and React allows developers to harness the power of fast builds, hot module replacement, and a highly efficient development workflow \cite{vite}. The three out of the four templates in Converty are also coded using React, Vite, and TypeScript. These templates provide the resulting online store that the shop owner configured. By using React, Vite, and TypeScript, the templates are able to leverage the benefits of these technologies, including component-based development, fast development, hot module replacement, and type safety. This ensures a consistent and efficient development process for creating and maintaining the shop owners' online shop.
...
\section{Explaining The Main Architecture}
After understanding the components that are present in the Figure \ref{fig:mainArchitecture} of the main architecture, it is crucial to have an understanding of how these components interact with each other.


The system ensures efficient communication between components for order management and admin control. Delivery companies send requests to the Go-based Delivery Service, which updates order statuses in MongoDB. The Master database handles writes, while slaves handle reads, with Redis providing cached data for faster access. The React Admin Frontend communicates with the Node.js Admin Backend, which interacts with the database and manages image storage via S3, delivering content through a Google CDN for optimized performance. This structure supports scalability and real-time data processing.
